...
function startGUI () {
    ...
    gui.add(config, 'SIM_RESOLUTION', { '32': 32, '64': 64, '128': 128, '256': 256 }).name('Разрешение').onFinishChange(initFramebuffers);
    gui.add(config, 'DENSITY_DISSIPATION', 0, 4.0).name('Время растворения');
    gui.add(config, 'VELOCITY_DISSIPATION', 0, 4.0).name('Вязкость');
    gui.add(config, 'PRESSURE', 0.0, 1.0).name('Давление');
    gui.add(config, 'CURL', 0, 50).name('Турбулентность').step(1);
    gui.add(config, 'SPLAT_RADIUS', 0.01, 1.0).name('Радиус кисти');
    gui.add(config, 'SHADING').name('Тени').onFinishChange(updateKeywords);
    gui.add(config, 'COLORFUL').name('Переливание цвета');
    gui.add(config, 'PAUSED').name('Стоп').listen();

    gui.add({ fun: function () {
        splatStack.push(parseInt(Math.random() * 20) + 5);
    } }, 'fun').name('Случайные мазки');

    var bloomFolder = gui.addFolder('Свечение');
    bloomFolder.add(config, 'BLOOM').name('enabled').onFinishChange(updateKeywords);
    bloomFolder.add(config, 'BLOOM_INTENSITY', 0.1, 2.0).name('intensity');
    bloomFolder.add(config, 'BLOOM_THRESHOLD', 0.0, 1.0).name('threshold');

    var sunraysFolder = gui.addFolder('Лучи');
    sunraysFolder.add(config, 'SUNRAYS').name('enabled').onFinishChange(updateKeywords);
    sunraysFolder.add(config, 'SUNRAYS_WEIGHT', 0.3, 1.0).name('weight');

    var captureFolder = gui.addFolder('Сделать скрин');
